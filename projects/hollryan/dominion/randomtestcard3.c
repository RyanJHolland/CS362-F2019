...
int test() {
	...
	if (setup < 0) {
		// error
		printf("error with initializeGame() in randomtestcard3.c!\n\n");
	}
	else {
		printf("initializeGame() in randomtestcard3.c succeeded.\n\n");
	}
	...
	*/
	...
	// run the test
	int test = tributeCardEffect(0, choice1, 0, 0, state, handPos, 0);
	/*
	// check the results
	if (test == -1) {
		printf("\nTest returned -1 \n");
