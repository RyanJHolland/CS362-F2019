...
int test() {
	...
	if (setup < 0) {
		// error
		printf("error with initializeGame() in randomtestcard1.c!\n\n");
	}
	else {
		printf("initializeGame() in randomtestcard1.c succeeded.\n\n");
	}
	...
	*/
	...
	int choice1 = randInRange(0, 1); // 1 is discard estate, 0 is draw estate
	...
	// run the test
	int test = baronCardEffect(0, choice1, 0, 0, state, handPos, 0);
